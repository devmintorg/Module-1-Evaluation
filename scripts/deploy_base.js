...
async function main() {
  ...
  await staker.connect(addr2).stake({value: hre.ethers.utils.parseEther("10")});
  await staker.connect(addr3).stake({value: hre.ethers.utils.parseEther("100")});
  await expect(staker.connect(addr4).stake({value: hre.ethers.utils.parseEther("1000")}))
    .to.be.revertedWith("Sent Too Much ETH. Maximum 100 ETH");
  await expect(staker.connect(addr4).stake({value: 1000}))
    .to.be.revertedWith("Sent Too Little ETH. Minimum 1 ETH");
  await expect(staker.connect(addr1).stake({value: hre.ethers.utils.parseEther("1")}))
  ...
  await expect(staker.connect(addr1).withdrawStake())
    .to.be.revertedWith("Cannot Withdraw Yet")
  await expect(staker.connect(addr2).withdrawReward())
    .to.be.revertedWith("Cannot Withdraw Yet")
  await expect(staker.connect(addr3).stake({value: hre.ethers.utils.parseEther("1")}))
    .to.be.revertedWith("You cannot stake!")
  ...
  await expect(staker.connect(addr2).stake({value: hre.ethers.utils.parseEther("1")}))
    .to.be.revertedWith("You cannot stake!")
  ...
  // Try to re-withdraw
  await expect(staker.connect(addr1).withdrawStake()).to.be.revertedWith("No Value to Unstake")
  await expect(staker.connect(addr1).withdrawReward()).to.be.revertedWith("No Stake Reward to Claim")
